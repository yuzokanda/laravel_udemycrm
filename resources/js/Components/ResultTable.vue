...
const priceNotation = (total) => Number(total).toLocaleString();
...
<template>
    <div class="lg:w-2/3 w-full mx-auto overflow-auto">
        <table v-if="data.type === 'perDay' || data.type === 'perMonth' || data.type === 'perYear'"  class="table-auto w-full text-left whitespace-no-wrap">
            <thead>
            <tr>
            ...
            <tr v-for="item in data.data" :key="item.date">
                ...
                <td class="border-b-2 border-gray-200 px-4 py-3">{{ priceNotation(item.total) }}</td>
            </tr>
            </tbody>
        </table>
    </div>
    <div class="lg:w-2/3 w-full mx-auto overflow-auto">
        <table v-if="data.type === 'decile'" class="table-auto w-full text-left whitespace-no-wrap">
            <thead>
            <tr>
            ...
            <tr v-for="item in data.data" :key="item.date">
                ...
                <td class="border-b-2 border-gray-200 px-4 py-3">{{ priceNotation(item.average) }}</td>
                <td class="border-b-2 border-gray-200 px-4 py-3">{{ priceNotation(item.totalPerGroup) }}</td>
